...
function _matrix_exp_gpu(A::CuArray{Complex{T}, 2}) where {T <: Real}
    E = _matrix_exp_gpu(reshape(A, size(A, 1), size(A, 2), 1))
    return reshape(E, size(A))
end

function _matrix_exp_gpu(A::CuArray{Complex{T}, 3}) where {T <: Real}
    n, m, batch = size(A)
    n == m || throw(DimensionMismatch("matrix exponential requires square matrices, got ($n, $m, $batch)"))

    I_n = reshape(CuArray(Matrix{Complex{T}}(I, n, n)), n, n, 1)
    I_batch = similar(A)
    I_batch .= I_n
